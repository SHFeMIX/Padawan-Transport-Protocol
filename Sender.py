...
class SendSocket(PTPSocket):
    ...
    def main_threading(self):
        ...
        while True:
            ...
            # 不能拿出来，只能看
            next = self.send_buffer[0]
            if not self.unack_buffer.empty():
                #print(f'长度{len(self.unack_buffer.queue)} 下一个是{next.getseq()} unack头是{self.unack_buffer.queue[0].getseq()}')
                print('')
                # 如果满了且不是重传的
                if self.unack_buffer.full() and next.getseq() > self.unack_buffer.queue[0].getseq():
                    continue
            ...
            if not self.timer:
                #print('开始计时')
                self.timer = threading.Timer(self.timeout, self.overtime_retrans)
                self.timer.start()
            if n > self.pdrop or not segment.data:
                # 写log snd
                #print(f'发的是 {segment.getseq()} {segment.getack()}')
                self._send(segment)
            else:
                # 写log drop
                #print(f'{segment.getseq()}丢包了')
                self.amount['Drop'] += 1
                self.logfile.write(segment.loginfor('drop', self.start_time))

            # resend segment doesn't need to put in unack buffer again
            if self.unack_buffer.empty() or segment.getseq() > self.unack_buffer.queue[0].getseq():
                #print(f'放入unack的是{segment.getseq()}')
    ...
    def receive_threading(self):
        ...
        while True:
            ...
            if ackdict[acknum] > 0:
                ...
                #print(f"重复总数是{self.amount['DupACK']}")
            else:
                # new ack
                self.timer.cancel()
                if not self.unack_buffer.empty():
            ...
            ackdict[acknum] += 1
            if ackdict[acknum] >= 3:
                #print(f'快速重传{head.getseq()}')
                # head.setresend()
                self.amount['Retrans'] += 1
                self.send_buffer.appendleft(head)

    def overtime_retrans(self):
        self.amount['Retrans'] += 1
        if self.unack_buffer.empty():
            return
        #print(f'超时重传{self.unack_buffer.queue[0].getseq()}')
        self.send_buffer.appendleft(self.unack_buffer.queue[0])


# 每次发包，如果计时器开启了就不管他，计时器没开启就开启
# 到达timeout时限后，重传unack的第一个(unack_buffer.pop())
# sender 需要两个线程同时开始，并阻塞主线程，两个都结束后再继续主线程
# 把receiving 做成sending的守护线程（失败）
# 实现超时重传
